...
int task2(void) {
    ...
    if(key >= 0 && key <= 26)    {                                                              //checks for an appropriate key
        for(i = 0; i < strlen(str); i++)   {                                                    //converts each letter of the input string to the encrypted letter
            if(str[i] >= 97 && str[i] <=122)    {                                               //checks for lower-case letters
                if(str[i] - key < 97)    {                                                      //checks if the letter is bigger than 'z'
                    str[i] += 26;                                                               //adds 26 to shift 'a - 1' into 'z'
                }
